...
for spk in os.listdir(args["input"]:
    for word in range(1, 11):
        for i in range(1, 11):
            ...
            mouth_destination_path = os.path.join(args["output"], spk, "words", "%02d"%word, "%02d"%i)
            if not os.path.exists(mouth_destination_path):
               os.makedirs(mouth_destination_path)

            inputparameters = {}
            outputparameters = {}
            reader = skvideo.io.FFmpegReader(os.path.join(args["input"], spk, "words", "%02d"%word, "%02d"%i, "word%02dutterance%02d.mp4"%(word, i)),
            ...
            writer = skvideo.io.FFmpegWriter(os.path.join(args["output"], spk, "words", "%02d"%word, "%02d"%i))
                ...
                if len(detections) > 0:
                    for k, d in enumerate(detections):
                        ...
                        if X_left_crop >= 0 and Y_left_crop >= 0 and X_right_crop < w and Y_right_crop < h:
                            ...
                            cv2.imwrite(os.path.join(mouth_destination_path, 'color_' + '_' + '%03d.jpg'%(counter+1), mouth_gray))
